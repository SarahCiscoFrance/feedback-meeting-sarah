...
exports.startFeedbackProcess = (req, res) => {
  var options = {
    method: "GET",
    url:
      process.env.codecsManagerAPI +
      "/codecs/mac/" +
      req.body.Event.Identification.MACAddress.Value
  };
...
function registerEvents(xapi, id) {
  ...
  xapi.event.on("Standby", function(event) {
    xapi
      .command("UserInterface Message Prompt Clear", {
        FeedbackId: "callrating-" + id
      })
      .catch(function(error) {
        console.error(error);
  ...
  xapi.event.on("UserInterface Message Prompt Response", function(event) {
    if (event.FeedbackId === "callrating-" + id) {
      updateCall(
        ...
        function() {
          xapi
            .command("UserInterface Message TextInput Display", {
              Duration: 0,
